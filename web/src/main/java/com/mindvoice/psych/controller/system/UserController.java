...
public class UserController {
    ...
    public CommonResult<PageResult<UserPageVO>> getUserPage(@Valid UserPageQuery queryParams) {
        PageResult<UserPageVO> result = userService.getUserPage(queryParams);
    ...
    public CommonResult<?> saveUser(@RequestBody @Valid UserForm userForm) {
    ...
    public CommonResult<UserForm> getUserForm(@Parameter(description = "用户ID") @PathVariable Long userId) {
    ...
    public CommonResult<Void> updateUser(@Parameter(description = "用户ID") @PathVariable Long userId, @RequestBody @Valid UserForm userForm) {
    ...
    public CommonResult<Void> deleteUsers(@Parameter(description = "用户ID，多个以英文逗号(,)分割") @PathVariable String ids) {
    ...
    public CommonResult<Void> updateUserStatus(@Parameter(description = "用户ID") @PathVariable Long userId, @Parameter(description = "用户状态(1:启用;0:禁用)") @RequestParam Integer status) {
        boolean result = userService.update(new LambdaUpdateWrapper<User>().eq(User::getId, userId).set(User::getStatus, status));
        return Result.judge(result);
    }
    ...
    public CommonResult<CurrentUserDTO> getCurrentUser() {
    ...
    public CommonResult<ExcelResult> importUsers(MultipartFile file) throws IOException {
...
//        ExcelUtils.importExcel(file.getInputStream(), UserImportDTO.class, listener);
    ...
    public void exportUsers(UserPageQuery queryParams, HttpServletResponse response) throws IOException {
        ...
        EasyExcel.write(response.getOutputStream(), UserExportDTO.class).sheet("用户列表").doWrite(exportUserList);
    }
    ...
    public CommonResult<?> updateUserProfile(@RequestBody UserProfileForm formData) {
    ...
    public CommonResult<?> resetPassword(@Parameter(description = "用户ID") @PathVariable Long userId, @RequestParam String password) {
    ...
    public CommonResult<?> changePassword(@RequestBody PasswordUpdateForm data) {
    ...
    public CommonResult<?> sendMobileCode(@Parameter(description = "手机号码", required = true) @RequestParam String mobile) {
    ...
    public CommonResult<?> bindOrChangeMobile(@RequestBody @Validated MobileUpdateForm data) {
    ...
    public CommonResult<Void> sendEmailCode(@Parameter(description = "邮箱地址", required = true) @RequestParam String email) {
    ...
    public CommonResult<?> bindOrChangeEmail(@RequestBody @Validated EmailUpdateForm data) {
...
//        List<USer> list = userService.listUserOptions();
