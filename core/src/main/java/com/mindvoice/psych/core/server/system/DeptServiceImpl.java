...
import com.mindvoice.psych.system.entity.Dept;
import com.mindvoice.psych.system.mapper.DeptMapper;
...
public class DeptServiceImpl implements DeptService {
    ...
    public List<DeptVO> getDeptList(DeptQuery queryParams) {
        ...
        // 查询数据
        List<Dept> deptList = deptMapper.selectList(new LambdaQueryWrapper<Dept>().like(StrUtil.isNotBlank(keywords), Dept::getName, keywords).eq(status != null, Dept::getStatus, status).orderByAsc(Dept::getSort));
    ...
    public List<DeptVO> recurDeptList(Long parentId, List<Dept> deptList) {
        return deptList.stream().filter(dept -> dept.getParentId().equals(parentId)).map(dept -> {
            DeptVO deptVO = BeanUtils.toBean(dept,DeptVO.class);
    ...
    public List<Option<Long>> listDeptOptions() {
        ...
        List<Dept> deptList = deptMapper.selectList(new LambdaQueryWrapper<Dept>().eq(Dept::getStatus, StatusEnum.ENABLE.getValue()).select(Dept::getId, Dept::getParentId, Dept::getName).orderByAsc(Dept::getSort));
        if (CollectionUtil.isEmpty(deptList)) {
            return Collections.EMPTY_LIST;
        }
    ...
    public Long saveDept(DeptForm formData) {
        ...
        long count = deptMapper.selectCount(new LambdaQueryWrapper<Dept>().eq(Dept::getCode, code));
        Assert.isTrue(count == 0, "部门编号已存在");

        // form->entity
//        Dept entity = deptConverter.toEntity(formData);
        Dept entity = BeanUtils.toBean(formData, Dept.class);
        ...
        // 保存部门并返回部门ID
        boolean result = SqlHelper.retBool(deptMapper.insert(entity));
    ...
    public DeptForm getDeptForm(Long deptId) {
        Dept entity = deptMapper.selectById(deptId);
        return BeanUtils.toBean(entity, DeptForm.class);
    }
    ...
    public Long updateDept(Long deptId, DeptForm formData) {
        // 校验部门名称/部门编号是否存在
        String code = formData.getCode();
        long count = deptMapper.selectCount(new LambdaQueryWrapper<Dept>().ne(Dept::getId, deptId).eq(Dept::getCode, code));
        Assert.isTrue(count == 0, "部门编号已存在");
        ...
        // 保存部门并返回部门ID
        boolean result = SqlHelper.retBool(deptMapper.updateById(entity));
    ...
    public boolean deleteByIds(String ids) {
        ...
        if (StrUtil.isNotBlank(ids)) {
            ...
            for (String deptId : menuIds) {
                deptMapper.update(new LambdaUpdateWrapper<Dept>().eq(Dept::getId, deptId).or().apply("CONCAT (',',tree_path,',') LIKE CONCAT('%,',{0},',%')", deptId).set(Dept::getIsDeleted, 1).set(Dept::getUpdateBy, SecurityUtils.getUserId()));
            }
        }
        return true;
    }
